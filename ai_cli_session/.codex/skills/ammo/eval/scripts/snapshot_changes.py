...
def _find_campaign_worktrees(
    ...
    # Also check for additional worktrees under .codex/worktrees/ that match
    ...
    if session_ids:
        for wt_parent in [repo_root / ".codex" / "worktrees"]:
...
def snapshot_changes(
    ...
    skill_diff = _run_git(
        ["diff", "HEAD", "--", ".codex/skills/ammo/"], cwd=str(repo_root)
    ) or ""
    if skill_diff:
