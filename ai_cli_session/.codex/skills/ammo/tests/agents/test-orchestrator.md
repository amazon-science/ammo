...
(`ai_cli_session/.codex/schemas/state.schema.json`, `$id = ammo.campaign.state/v2`).
...
Codex runtime note: references to a "round team" or `team_name` mean the
logical round agent cohort tracked by the lead; implement them with
`spawn_agent`, `send_input`, and `close_agent` on the relevant
champion/implementer/monitor pairs rather than Claude TeamCreate/TeamDelete
objects.

## How to Run

```
Run the AMMO orchestrator conformance tests. Spawn Codex subagents that:
1. Read these files first:
   - .codex/skills/ammo/SKILL.md
   - .codex/skills/ammo/orchestration/parallel-tracks.md
   - .codex/skills/ammo/orchestration/integration-logic.md
   - .codex/skills/ammo/orchestration/debate-protocol.md
...
```
Context: Two impl tracks are running (implementer agents) in the round team.
...
2. As each implementer returns, run its compilation gate (T9) in the track worktree.
...
5. When all current-round tracks are terminal and audit gates allow, close every active round agent/monitor pair (`ammo-round-2-llama70b-h100`).
...
2. Read `{artifact_dir}/rounds/1/tracks/op001/validation_results.md` and ensure `campaign.rounds[0].parallel_tracks.tracks.op001` carries structured status/metrics (T10).
3. Run the Stage 4-5 validation gate report.
4. Satisfy T11 (all tracks terminal), then close every active round agent/monitor pair (`ammo-round-1-llama70b-h100`).
...
- Leave active implementer/monitor agents open after all current-round tracks are terminal.
- Implement anything yourself.
...
Expected behavior: close every stale round agent/monitor pair with `close_agent` before recreating the logical round cohort. Re-create from scratch. Do NOT skip debate.
...
4. Close every stale round agent/monitor pair with `close_agent` for the stale logical round cohort.
...
- op003: `status: "GATED_PASS"`, `verdict: "GATED_PASS"`, `e2e_speedup: 1.025`, `gating: {env_var: "VLLM_MOE_TWO_STREAM", crossover_threshold_bs: 16, regressing_bs: [32]}`, modifies `vllm/model_executor/layers/fused_moe/fused_moe.py`
...
3. Spawns DA reviewer (Codex) to verify resolver's merge.
...
**Next actions (in order):**
1. Do NOT touch the code. Re-spawn a new ammo-implementer into the same worktree, providing the error message and context so the implementer can apply the fix itself.
...
2. After round 1 Phase C, check champion open-items declarations. If any declare open items, run round 2.
3. Run at least 1 full debate round (A/B/C + open items declaration).
4. Write summary.md, select winners, close debate champions with `close_agent`. Round team persists for Stages 4-5.

**Must NOT do:**
- Skip the debate.
- Unilaterally declare flash_attn_fwd the winner.
- Treat "obvious" dominance as a fast-track exception.
- close the round team itself after debate — the logical round cohort persists for implementation agents in Stages 4-5.

**Skill reference:**
- debate-protocol.md § "Debate is Always Mandatory": "There is no fast-track exception."
- SKILL.md § Stage 3: "After selection: close debate champions with `close_agent`... The round team persists."
</details>
...
**Next actions (in order):**
1. The Codex research review guard catches the production-parity violation (`--enforce-eager` is one of its checks) and writes `research_review.json` with `{ok: false}`. The researcher must re-run.
...
Context: Implementer returned `verdict: "PASS"`. validation_results.md shows: `Command: vllm bench latency --model meta-llama/Llama-3-70B --batch-size 1 --num-iters 50`. Results look good — 12.7% improvement.
...
Context: Implementer returned `verdict: "PASS"`. validation_results.md notes: `Environment: VLLM_TORCH_COMPILE_LEVEL=0 (to isolate kernel improvement from compile effects)`. E2E: 1.174x speedup.
...
4. Re-spawn implementer to re-run ONLY the E2E validation with the correct environment.
