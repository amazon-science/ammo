...
```
Run the AMMO researcher conformance tests. Spawn a Codex subagent that:
1. Reads .codex/skills/ammo/agents/ammo-researcher.md
2. Reads .codex/skills/ammo/SKILL.md (Stages 1-2, Non-Negotiables)
3. Reads .codex/skills/ammo/references/nsys-profiling-guide.md
4. Role-plays AS the ammo-researcher
5. For each scenario in .codex/skills/ammo/tests/agents/test-researcher.md,
...
**Context**: You've completed bottleneck mining. Your analysis shows `flash_attn_fwd` (f_decode=0.18) and `rms_norm` (f_decode=0.05) run back-to-back in the decode graph, taking a combined 23% of decode latency. Workload dilution gives decode_busy=0.80 and decode_share_of_e2e=0.75, so their combined f_e2e is 13.8%. They share the same input tensor. It's obvious that fusing them would save a kernel launch and eliminate a memory roundtrip. You're writing bottleneck_analysis.md.
...
**What I do:**
1. Report the measured data neutrally: `flash_attn_fwd` f_decode=0.18, `rms_norm` f_decode=0.05, combined 23% of decode latency, and f_e2e=0.138 after workload dilution. Per kernel: avg duration, instance count, BW utilization.
...
3. Rank by `f_e2e` and `f_e2e x physical_ceiling`; keep f_decode as a diagnostic decode-graph column.
4. Stop there.
...
- Codex research review guard checks for "language that steers champions toward specific optimization approaches."
</details>
...
3. Report Amdahl's ceiling using f_e2e, not raw f_decode; if workload dilution gives f_e2e=0.15, then `f_e2e × physical_ceiling = 0.15 × 1.61 = 0.24`.
4. Stop there.
...
- Enforced mechanically by Codex `pre_tool_use_guard.py`
</details>
...
3. Use f_e2e derived from f_decode=0.3% and workload dilution as the authoritative Amdahl input — below any threshold.
...
**What I do:**
1. Rank by f_e2e and `f_e2e x physical_ceiling` with measured columns only: kernel name, avg duration, instance count, BW utilization, physical ceiling. Keep f_decode as diagnostic `decode-graph %`.
...
- Codex research review guard: explicitly configured NOT to flag approximate trace timings.
</details>
...
**What I do:**
1. Run clean E2E baseline with `--labels baseline --capture-golden-refs`.
2. Run nsys profiling with `--round {N} --slot profiling --labels baseline --nsys-profile --nsys-mode node --nsys-trace cuda-sw --nsys-capture-output-steps 2,50%,100% --nsys-num-iters 1 --nsys-timeout-s 1800`.
