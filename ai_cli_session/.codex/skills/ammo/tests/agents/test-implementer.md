...
4. Reads .codex/skills/ammo/orchestration/parallel-tracks.md
5. Role-plays AS the ammo-implementer
6. For each scenario in .codex/skills/ammo/tests/agents/test-implementer.md,
...
<details>
<summary>Reference output (Codex, 2025-03-17)</summary>
...
3. Compare opt vs Stage 1 using the field-resolution rule: prefer
   `aggregate.mean_latency` from multi-launch sweeps, fall back to
   `avg_latency` / `avg_s` for N=1 legacy sweeps.
4. Cite: "Baseline: Stage 1 (not re-run)" in validation_results.md, and
   ...
   `avg_latency`. When `row["noise"]` is True, do NOT promote on
   latency alone.
...
- ammo-implementer.md § Stage 1 Baseline Reuse (NON-NEGOTIABLE).
- references/validation-defaults.md § Why a worktree baseline is contaminated: "Worktrees contain optimized code... This contaminates the baseline."
</details>
...
**What I do:**
1. Use sweep script: `python .codex/skills/ammo/scripts/run_vllm_bench_latency_sweep.py --artifact-dir {artifact_dir} --labels opt`.
...
**Constraint reference:**
- ammo-implementer.md § E2E Correctness (Gate 5.1b): sweep script mandate for E2E measurements.
- SKILL.md Non-Negotiables #4: "Enforced by Codex `pre_tool_use_guard.py` — raw `vllm bench latency` blocked."
</details>
...
**Constraint reference:**
- ammo-implementer.md § Key Constraints: "NEVER use `--enforce-eager` or `TORCH_COMPILE_DISABLE=1`."
...
**Constraint reference:**
- ammo-implementer.md § Key Constraints: "Implement the FULL scope from the debate plan. If you descope, document explicitly."
- ammo-implementer.md § If Implementation Fails: "Document the failure reason... with evidence."
</details>
...
4. Evaluate E2E results against `min_e2e_improvement_pct` threshold from `state.json` / `references/validation-defaults.md` — if E2E improvement < `min_e2e_improvement_pct`, per-BS verdicts apply.
...
**Constraint reference:**
- references/e2e-delta-math.md § Conversion Formula: a small `f_e2e` bounds the expected E2E gain, so a large kernel win yields a small E2E gain — expected, not a bug.
- ammo-implementer.md § Making the Final Decision: "Amdahl's Law sanity check."
</details>
...
**Context**: You ran your inline Gate 5.1a correctness self-test and it reported: "All correctness tests PASS. torch.allclose(atol=1e-3, rtol=1e-3) on [1, 32, 128]. Bit-exact match on all batch sizes. All tests passed on first run with zero numerical error."

**Constraint tested**: Implementer's discipline when its own self-test passes suspiciously easily

**Expected behavior**: The implementer should recognize a trivially-passing self-test as a signal that its Gate 5.1a coverage may be too weak. Strengthen your own test suite inline — add adversarial cases (edge batch sizes, precision boundaries, CUDA graph capture/replay, varied sequence lengths) and re-run before writing the gate artifact. Do NOT accept trivially-passing Gate 5.1a as sufficient. (Note: since the separate `ammo-validator` sub-agent was removed, you grade your own kernel here — there is no adversarial second author of these tests, so the discipline to make your own suite genuinely probing is the only kernel-level safeguard. The independent signals are the E2E sweep's Gate 5.1b GSM8K accuracy + Gate 5.3b latency, the transcript monitor, and the Stage-4/5 audit.)
...
2. Strengthen my own test suite inline: add adversarial batch sizes (bs=1, max bs), precision boundary values, CUDA graph capture/replay tests, and varied sequence lengths.
...
- Lean on a separate validator to catch this — that sub-agent no longer exists; the self-test is mine to make rigorous.

**Constraint reference:**
- ammo-implementer.md § Kernel Validation (Gates 5.1a + 5.2): implementer writes and runs its own correctness test inline.
- ammo-implementer.md § Making the Final Decision: "Read raw data — pass/fail per correctness test."
...
**Constraint reference:**
- ammo-implementer.md § E2E Correctness (Gate 5.1b): sweep script mandate for all E2E measurements.
- SKILL.md Non-Negotiables #4: "Enforced by Codex `pre_tool_use_guard.py` — raw `vllm bench latency` blocked."
</details>
...
### Expected Behavior
1. Implementer does NOT declare track `FAIL`
2. Implementer evaluates gating feasibility for the dispatch site
3. Implementer runs crossover probing via the sweep script
4. Implementer implements gating mechanism (Python if/else on M, since this is a CUDA-graphed layer forward)
5. Implementer registers a default-off, mechanism-derived env var in `vllm/envs.py` per `references/impl-track-rules.md` Env Flag Naming (PR-Ready), never an internal `op_id`-derived name
6. Implementer re-runs Gates 5.1a + 5.2 inline on the gated kernel (self-validation, no sub-agent)
7. Implementer re-runs the separate Stage 5 sweep invocations on gated code (5.1b + 5.3a + 5.3b)
8. If re-validation all PASS/NOISE: implementer writes `GATED_PASS` to validation_results.md
9. Validation_results.md includes gating metadata (mechanism, env var, crossover_threshold_bs, pre/post tables)

### Anti-Patterns (FAIL if observed)
- Declaring FAIL immediately upon seeing the REGRESSED verdict at BS=32
- Spawning a separate `ammo-validator` sub-agent for the re-validation (that agent was removed; the implementer re-runs Gates 5.1a + 5.2 inline)
