...
class TestSweepFlagCombinations:
    """Verify invalid high-risk sweep flag combinations fail before execution."""

    def test_nsys_profile_cannot_combine_with_verify_correctness(self, tmp_path):
        from run_vllm_bench_latency_sweep import main
...
# ---------------------------------------------------------------------------
# _inject_nsys_execute_timeout_env
# ---------------------------------------------------------------------------


class TestNsysExecuteTimeoutEnv:
    """nsys profiling raises vLLM's internal worker RPC timeout."""

    def _fn(self):
        import run_vllm_bench_latency_sweep as sweep
        return sweep._inject_nsys_execute_timeout_env

    def test_sets_timeout_when_absent(self):
        inject = self._fn()
        out = inject({"PATH": "/usr/bin"}, 1800)
        assert out["VLLM_EXECUTE_MODEL_TIMEOUT_SECONDS"] == "1800"

    def test_preserves_larger_existing_timeout(self):
        inject = self._fn()
        out = inject({"VLLM_EXECUTE_MODEL_TIMEOUT_SECONDS": "2400"}, 1800)
        assert out["VLLM_EXECUTE_MODEL_TIMEOUT_SECONDS"] == "2400"

    def test_replaces_invalid_existing_timeout(self):
        inject = self._fn()
        out = inject({"VLLM_EXECUTE_MODEL_TIMEOUT_SECONDS": "bad"}, 1800)
        assert out["VLLM_EXECUTE_MODEL_TIMEOUT_SECONDS"] == "1800"
    ...
    def test_does_not_mutate_input(self):
        ...
        out = inject(env, 1800)
        assert env["VLLM_EXECUTE_MODEL_TIMEOUT_SECONDS"] == "300"
        assert out["VLLM_EXECUTE_MODEL_TIMEOUT_SECONDS"] == "1800"
