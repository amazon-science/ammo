...
- `agents/ammo-implementer.md` — per-category validation gate routing + scaffolds (the impl-champion runs the kernel correctness & speedup checks)
...
The champion selects the category in **Phase 0**, after it analyzes the full profile. Each of a champion's 2-3 ranked candidates declares its own category. See `agents/ammo-champion.md` § Candidate Eligibility and Generation and `orchestration/debate-protocol.md` § Phase 0: Independent Proposals.
...
The lead's eligibility check in `orchestration/debate-protocol.md` § Eligibility, revision, and elimination validates the Category block on schema_version ≥ 4.1 campaigns. These fields are required, verbatim from `agents/ammo-champion.md`:
