...
Example:
  .venv/bin/python .codex/skills/ammo/scripts/new_target.py \\
...
def _state_json(fields: TargetFields, artifact_dir: Path, min_e2e_improvement_pct: float | None = None,
    ...
    # Server/API campaign identity and Codex root-thread identity are distinct.
    session_id = os.environ.get("AMMO_SESSION_ID")
    codex_thread_id = os.environ.get("CODEX_SESSION_ID")
    if not codex_thread_id:
        identity_path = (
            Path(os.environ.get("AMMO_GPU_RES_DIR") or "/tmp/ammo_gpu_res").expanduser()
            / "codex_hook_session.json"
        )
        try:
            identity_doc = json.loads(identity_path.read_text(encoding="utf-8"))
            if identity_doc.get("server_session_id") == session_id:
                codex_thread_id = identity_doc.get("codex_thread_id")
