...
_AUDIT_STARTED_SCRIPT = ".claude/skills/ammo/scripts/ammo_state.py"
...
def find_schema(start_path):
    """Walk up from `start_path` to locate .claude/schemas/state.schema.json,
    ...
    if d.is_file():
        d = d.parent
    for _ in range(10):
        candidate = d / ".claude" / "schemas" / "state.schema.json"
        ...
        if audit_key:
            ...
            if not passed:
                return (
                    ...
                    "for %s first (see .claude/skills/ammo/orchestration/audit-protocol.md)."
        ...
        if mining.get("completed_at"):
            ...
            if missing:
                return (
                    ...
                    "`.venv/bin/python .claude/skills/ammo/scripts/ammo_state.py backfill "
...
def _build_socratic(**kw):
    ...
    if (not socratic and stage == "4_5_parallel_tracks"
        ...
        socratic = (
            "REASON THROUGH THIS: You're about to shut down round-team agents via SendMessage "
            "shutdown_request. Once they are gone, track context exists only in artifact files.\n\n"
...
def _build_reminder(**kw):
    ...
    if stage == "3_debate":
        if not kw["team_name"]:
            return "Next: spawn 2-4 ammo-champion (named) into the implicit team (no TeamCreate). No monitors for debate."
        ...
        if not kw["tracks_started"]:
            return ("Winners selected. Shut down each debate champion (SendMessage shutdown_request); "
                    "confirm each shutdown_approved (bounded 5-min wait per shutdown-protocol.md; idle "
                    "transcript = drained) before spawning ammo-impl-champion + monitor per winner.")
        return ""
    ...
    if stage == "4_5_parallel_tracks":
        ...
        if kw["track_count"] > 0 and kw["tracks_non_terminal"] == 0 and not kw["integ_started"]:
            ...
            if kw["tracks_passing"] == 1:
                return ("All tracks terminal (1 passer). Shut down round-team agents via SendMessage "
                        "shutdown_request, confirm shutdown_approved (bounded 5-min wait per "
                        "shutdown-protocol.md; idle transcript = drained), then Stage 6 single-track short-circuit: "
                        ...
                        "Short-Circuit.")
            return ("All tracks terminal. Shut down round-team agents via SendMessage shutdown_request, "
                    "confirm shutdown_approved, then proceed to Stage 6 integration (file-set conflict "
                    "analysis). Bounded 5-min wait per shutdown-protocol.md; idle transcript = drained.")
        return ""
    ...
    if stage.startswith("7_campaign_eval"):
        if audit_exists and not kw["audit_s67"]:
            return ("AUDIT REQUIRED (T_AUDIT_S67): audit.stage_67.passed_at not set. Spawn ammo-auditor "
                    "(4-phase: inventory → reconstruction → checklist → reconciliation). "
                    "Stage: stage_67.")
        return ("Shut down any remaining round-team agents (SendMessage shutdown_request, confirm each "
                "shutdown_approved under the bounded 5-min wait of shutdown-protocol.md, idle transcript "
                "= drained — the implicit team auto-cleans on session exit; no TeamDelete). "
                "Mechanical check: compare "
