...
This file is the lead's contract. Role behavior lives in `.claude/agents/`, stage choreography in `orchestration/`, and domain policy in `references/`. Open a linked file when its stage or decision becomes active. Do not preload the whole corpus.
...
2. `.claude/schemas/state.schema.json` and `scripts/transitions.json` for state shape, enums, and legal transitions.
3. The topical `references/*.md` file.
4. The stage's `orchestration/*.md` file.
5. The role's own `.claude/agents/*.md` file.
...
- Talk to live teammates with `SendMessage`; plain text in the lead thread does not reach them. To retire a teammate, follow `references/shutdown-protocol.md`.
...
Collect every parameter the user gave. Scaffold once with `.venv/bin/python .claude/skills/ammo/scripts/new_target.py`, and omit only the flags the user did not set. `references/workload-invocation.md` owns defaults, workload-matrix handling, `max_num_seqs`, DP/EP semantics, and the canonical command.
...
|---|---|---|---|
| `1_baseline` | lead + fresh `ammo-researcher` | clean production-parity baseline/golden capture and a separate bounded attribution profile | `.claude/agents/ammo-researcher.md`, `references/nsys-profiling-guide.md` |
| `2_bottleneck_mining` | lead + `ammo-researcher` | grounded `bottleneck_analysis.md` with `## Technology Landscape`, workload dilution, component shares, and physical bounds | `.claude/agents/ammo-researcher.md`, `references/e2e-delta-math.md` |
...
| `4_5_parallel_tracks` | one `ammo-impl-champion` and one paired `ammo-transcript-monitor` per winner | isolated implementation, complete gates, a terminal verdict, durable evidence | `orchestration/parallel-tracks.md` |
...
| `7b_report` | fresh `ammo-report-writer` + fresh adversarial fact-checker | artifact-grounded `REPORT.md` accepted with `ok:true` | `.claude/agents/ammo-report-writer.md` |

`ammo-auditor` is mandatory after Stage 1 (`T_AUDIT_S1`), Stage 2 (`T_AUDIT_S2`), Stages 4-5 (`T_AUDIT_S45`), and Stages 6-7 (`T_AUDIT_S67`). Auditors may hand bounded evidence tasks to `ammo-delegate`. `ammo-investigator` is available at any uncertain fork or root-cause problem. The implementation role is registered as `ammo-impl-champion` and the report role as `ammo-report-writer`.

**Team-member vs subagent contract (HARD):** Only `ammo-champion`, `ammo-impl-champion`, and `ammo-transcript-monitor` are team members. Spawn each one with a unique `name` and `run_in_background=True`. The first named spawn forms the session's single implicit team, and later named spawns join it — there is no TeamCreate, and `team_name` is deprecated and ignored. While a teammate's round is active, reuse it by addressing its `name` with `SendMessage`. Never re-spawn a `name` that is still on the roster. Put the campaign round and a bounded attempt suffix in any name that can recur. When a business identifier such as `OP-001` names a track, keep the raw `op_id` in state, prompts, monitor ledgers, and artifact paths. All other roles — `ammo-researcher`, `ammo-auditor`, `ammo-investigator`, `ammo-delegate`, `ammo-report-writer`, and `ammo-resolver` — are one-shot subagents: spawn them with `subagent_type` + `prompt` only, and no `name`. `state.json.team_name` is descriptive bookkeeping, never a runtime handle.
...
For each winner, spawn one `ammo-impl-champion` (`isolation: worktree`, so the Agent tool creates the track's own branch and worktree) and one read-only `ammo-transcript-monitor`. Record each track's stable identity in state. The monitor reviews methodology continuously and runs its own collision checks; it does not implement and does not author the track verdict.
...
At `campaign_complete` or `campaign_exhausted`, spawn a fresh `ammo-report-writer`. It must build every factual claim from campaign artifacts, disclose missing or failed evidence, and get `ok:true` from a fresh adversarial fact-checker before returning the report.
...
`.claude/schemas/state.schema.json` owns the closed state shape and enums. `scripts/transitions.json` owns legal transitions. Do not restate or invent enum values in prose. Keep these stage values exactly:
